...
<Card.Root class={"overflow-hidden " + ($$props.class || "")}>
      ...
      <Tooltip.Root openDelay={333}>
        ...
        <Tooltip.Content>
          <p class="max-w-[310px] text-wrap">
            Key terms from the point of view of satisfied customers, highlighting how they perceive your product, based
            on positive feedbacks posted in the selected period.
          </p>
        </Tooltip.Content>
      </Tooltip.Root>
    </Card.Title>
    <UserRoundPlus class="h-4 w-4 shrink-0 text-muted-foreground" />
  </Card.Header>
  <Card.Content class="flex h-[232px] w-full flex-col items-center justify-center px-6 pb-4 pt-1">
    ...
    {:then}
      ...
      {:else}
        {@const metricData = data(metric.positive)}
        <div class="group relative flex h-full w-full items-center justify-center">
          {#if metricData.length}
            <WordCloud
              ...
              fontSize={size(metric.positive)}
              class={"shrink-0 fill-foreground opacity-100 transition-opacity " +
                (previousMetric && !isEqual(metric.positive, previousMetric.positive) ? "group-hover:opacity-25" : "")}
            />
          {:else}
          ...
          {/if}
          {#if previousMetric && !isEqual(metric.positive, previousMetric.positive)}
            {@const previousMetricData = data(previousMetric.positive)}
            <WordCloud
              values={previousMetricData}
              width={329}
              height={212}
              fontFamily={'"Geist Sans", "Geist Sans fallback", ui-sans-serif, system-ui, -apple-system, "system-ui", "Helvetica"'}
              fontSize={size(previousMetric.positive)}
