...
<Card.Root class={"overflow-hidden " + ($$props.class || "")}>
      ...
      <Tooltip.Root openDelay={333}>
        ...
        <Tooltip.Content>
          <p class="max-w-[310px] text-wrap">
            Key terms from the point of view of neutral customers, highlighting how they perceive your product, based on
            neutral feedbacks posted in the selected period.
          </p>
        </Tooltip.Content>
      </Tooltip.Root>
    </Card.Title>
    <UserRoundMinus class="h-4 w-4 shrink-0 text-muted-foreground" />
  </Card.Header>
  <Card.Content class="flex h-[232px] w-full flex-col items-center justify-center px-6 pb-4 pt-1">
    ...
    {:then}
      ...
      {:else}
        {@const metricData = data(metric.neutral)}
        <div class="group relative flex h-full w-full items-center justify-center">
          {#if metricData.length}
            <WordCloud
              ...
              fontSize={size(metric.neutral)}
              class={"shrink-0 fill-foreground opacity-100 transition-opacity " +
                (previousMetric && !isEqual(metric.neutral, previousMetric.neutral) ? "group-hover:opacity-25" : "")}
            />
          {:else}
          ...
          {/if}
          {#if previousMetric && !isEqual(metric.neutral, previousMetric.neutral)}
            {@const previousMetricData = data(previousMetric.neutral)}
            <WordCloud
              values={previousMetricData}
              width={329}
              height={212}
              fontFamily={'"Geist Sans", "Geist Sans fallback", ui-sans-serif, system-ui, -apple-system, "system-ui", "Helvetica"'}
              fontSize={size(previousMetric.neutral)}
