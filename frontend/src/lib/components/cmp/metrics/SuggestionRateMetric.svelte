<script lang="ts">
  ...
  let previousParams: entities.SuggestionCountParams;
  export let params: entities.SuggestionCountParams;
  let load: Promise<void>;
  let previousMetric: entities.SuggestionCountMetric | undefined;
  let metric: entities.SuggestionCountMetric;

  let loadMetric = debounce((params: entities.SuggestionCountParams) => {
    load = (async () => {
      let query = payloads
        ...
        .toString();
      let response = await api.get<payloads.GetSuggestionCountMetricResponse>(
        `/products/${params.productID}/metrics/suggestion-count?${query}`,
      );
      metric = payloads.toSuggestionCountMetric(response);
      ...
      response = await api.get<payloads.GetSuggestionCountMetricResponse>(
        `/products/${params.productID}/metrics/suggestion-count?${query}`,
      );
      previousMetric = payloads.toSuggestionCountMetric(response);
    })();
  }, 50);
...
<Card.Root class={"overflow-hidden " + ($$props.class || "")}>
      ...
      <Tooltip.Root openDelay={333}>
        ...
        <Tooltip.Content>
          <p class="max-w-[355px] text-wrap">
            How likely customers are to make a suggestion, and the current number identified in feedbacks posted in the
            selected period.
          </p>
        </Tooltip.Content>
      </Tooltip.Root>
  ...
  <Card.Content>
    ...
    {:then}
      ...
      {:else}
        {@const metricRate = rate(metric.activeSuggestions + metric.archivedSuggestions, metric.feedbacks)}
        <div class="text-2xl font-bold">
          {metricRate}%
          <span class="align-text-top text-lg font-medium text-muted-foreground">
            ({new Intl.NumberFormat("en-US").format(metric.activeSuggestions + metric.archivedSuggestions)})
          </span>
        </div>
        {#if previousMetric}
          {@const previousMetricRate = rate(
            previousMetric.activeSuggestions + previousMetric.archivedSuggestions,
            previousMetric.feedbacks,
          )}
