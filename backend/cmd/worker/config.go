...
func NewConfig() *config.Config {
	...
	config.Service.GracefulTimeout = 60 * time.Second
	...
	config.Database.MaxConns = min(8, 2*runtime.GOMAXPROCS(-1))
	...
	config.Cache.MaxConns = min(8, 2*runtime.GOMAXPROCS(-1))
	...
	config.Worker.Concurrency = min(8, 2*runtime.GOMAXPROCS(-1))
	config.Worker.StrictPriority = false
	config.Worker.StopTimeout = config.Service.GracefulTimeout
	config.Worker.HealthPort = util.GetEnv("CLANK_WORKER_HEALTH_PORT", 1112)
