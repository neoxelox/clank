  ...
  jobs: {
    ...
    engine: {
                ...
                attachments: [
                  {
                    color: "danger",
                    mrkdwn_in: ["text"],
                    text: "*`Engine`* containers are *unhealthy*, restarting...\n```" + message + "```",
                  },
                ],
              };
            case "HEALED":
                ...
                attachments: [
                  {
                    color: "warning",
                    mrkdwn_in: ["text"],
                    text: "*Failed* to heal *`engine`* containers, retrying...\n```" + error + "```",
                  },
                ],
              };
          }
        ...
        heal: async (docker, _) => {
          const find = (container, name) => _.find(container.Names, (cname) => cname.includes(name));
          const containers = await docker.listContainers();
          const engine = await docker.getContainer(_.find(containers, (container) => find(container, "engine")).Id);
